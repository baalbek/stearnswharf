...
createSZ :: P.Profile a => Beam a -> L.Load -> Vector Double
createSZ beam load = fromList [s0,s1,s2,s3,s4,s5]
    where q1 = (L.qz1 load)
          q2 = (L.qz2 load)
...
add2systemK m beam = mapM_ mywrite $ l2gMx beam
    where myk = trans curTg <> createK beam<> curTg 
          curTg = tg beam 
...
add2systemSZ' :: P.Profile a => STVector s Double -> L.LoadVariant -> Beam a -> ST s ()
add2systemSZ' vec loadVar beam = mapM_ mywrite $ indexSeeds beam 
    where syk = trans (tg beam) <> createSZ beam load
...
add2systemSZ :: P.Profile a => STVector s Double -> L.LoadVariant -> Beam a -> ST s ()
add2systemSZ vec loadVar beam = case ld beam of Nothing -> return ()
                                                Just _ -> add2systemSZ' vec loadVar beam 
